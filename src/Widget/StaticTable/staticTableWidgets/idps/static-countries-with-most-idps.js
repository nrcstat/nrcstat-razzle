...
export default function (widgetParams) {
  ...
  const title = t(
    `RefugeeReport${periodYear + 1}.IDP.CountriesWithMostIdps.Heading`
  );

  const footerAnnotations = t(
    `RefugeeReport${periodYear + 1}.IDP.CountriesWithMostIdps.TableFooterText`
  ).replace("\n", "<br /><br />");
  ...
  return generator(
    title,
    t(`RefugeeReport${periodYear + 1}.MiscSharedLabels.numberIdps`),
    process,
    query,
    footerAnnotations,
