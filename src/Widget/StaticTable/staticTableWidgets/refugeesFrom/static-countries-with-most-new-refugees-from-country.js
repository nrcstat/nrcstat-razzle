...
export default function (widgetParams) {
  ...
  const title = t(
    `RefugeeReport${
      periodYear + 1
    }.RefugeesFrom.CountriesWithMostNewRefugeesFromCountry.Heading`
  );

  const footerAnnotations = t(
    `RefugeeReport${
      periodYear + 1
    }.RefugeesFrom.CountriesWithMostNewRefugeesFromCountry.TableFooterText`
  );
  ...
  return generator(
    title,
    t(`RefugeeReport${periodYear + 1}.MiscSharedLabels.numberRefugees`),
    process,
    query,
    footerAnnotations,
